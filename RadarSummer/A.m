...
f7 = figure(7);plot(RealSignalAll,'r-');title('实信号回波');
grid on;zoom on;
saveas(f7,'总实回波信号（figure7）.fig')
...
f8=figure(8);%加噪声之后的总回波信号
plot(RealEchoAll,'r-');title('带杂波的实信号回波,闭锁期为0');
saveas(f8,'总回波实信号含杂波（figure8）.fig')
